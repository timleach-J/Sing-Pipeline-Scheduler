...
def _load_credentials() -> dict:
    """Load Climb API credentials.

    Canonical location is lib/credentials/sing_credentials.json — that folder is
    the ONLY home for secrets and is git-ignored wholesale. The old lib/ location
    is still accepted so nothing breaks mid-move, but it warns so the stray copy
    gets cleaned up rather than lingering.
    """
    import os as _os_cred
    _lib = _os_cred.path.dirname(_os_cred.path.abspath(__file__))
    _preferred = _os_cred.path.join(_lib, 'credentials', 'sing_credentials.json')
    _legacy    = _os_cred.path.join(_lib, 'sing_credentials.json')

    if _os_cred.path.exists(_preferred):
        cred_path = _preferred
        if _os_cred.path.exists(_legacy):
            print(f'  [!] A second copy of sing_credentials.json is still at {_legacy}')
            print( '      Using lib/credentials/. Delete the stray copy.')
    elif _os_cred.path.exists(_legacy):
        cred_path = _legacy
        ...
        raise FileNotFoundError(
            ...
            f'  {_preferred}\n'
            'Copy sing_credentials.json.template into lib/credentials/ as '
            'sing_credentials.json and fill in your client_id, client_secret, '
            'and workgroup_key.'
        )
...
def get_next_sample_number(verbose: bool = True) -> int:
    """
    Fetch all samples from Climb and return the next sequential sample number.
    Sample numbers are globally sequential across all projects, so no study
    filter is applied — find the highest number across everything and add 1.

    Handles all known name formats: plain int (1796), suffix (1796a, 1796b),
    range (1753-1754), old formats (295-295A, 346, 347).
    Uses regex to extract the leading integer so no valid sample is skipped.
    """
    import re as _re

    if verbose:
        print('Fetching samples from Climb to find next sample number...')
    raw = _get_all('/api/samples')
    if verbose:
        print(f'  Total samples in Climb: {len(raw)}')
    ...
    next_num = max_num + 1
    if verbose:
        print(f'  Highest sample number found: {max_num}  →  Next: {next_num}')
    return next_num
